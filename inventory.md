...
| `importer-porfolio` | 2026-09-15 | Opened [PR #11](https://github.com/fedefailla18/importer-porfolio/pull/11): `SyncJobsPanel` (new `syncJobsSlice`) showing live per-data-type status/retry for investracker PR #65's Binance sync-job model; `BinanceSyncDialog` stays open post-trigger instead of closing on a toast. `tsc`/lint/build all clean; not yet exercised in a browser against a live Binance account. [PR #10](https://github.com/fedefailla18/importer-porfolio/pull/10) (Dockerfile for `crypto-ui`) still open from before, awaiting review. |
...
| `investracker` | 2026-09-15 | Opened [PR #65](https://github.com/fedefailla18/investracker/pull/65): resumable, chunked job tracking (`sync_job`/`sync_job_chunk`) for the Binance full-history sync, replacing the old single-`@Transactional` fire-and-forget design — the fix is removing all ambient transactions from the job path (every save commits on its own), not adding finer-grained ones. Retry only re-runs FAILED chunks. Spec written before implementation: `docs/binance-sync-jobs.md`. Verified: `compileJava`/`compileTestGroovy` clean, new `BinanceFullSyncServiceSpec` (7 tests) + `TransactionServiceSpec` additions passing, full suite shows the same 13 pre-existing failures as `main` (stash A/B'd), 0 new failures. Also renamed `FileImporterApplication`→`InvestrackerApplication` (2026-09-13, direct-to-main exception, user's call) — last file still carrying the old project name. [PR #63](https://github.com/fedefailla18/investracker/pull/63)/[#64](https://github.com/fedefailla18/investracker/pull/64) still open from before, awaiting review. |
