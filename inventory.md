...
| `investracker` | 2026-09-25 | [PR #65](https://github.com/fedefailla18/investracker/pull/65) has grown into a running log of real bugs hit while dogfooding the Binance full-sync job model, each confirmed against the user's actual DB before being fixed: orphaned-job self-healing on restart (`reconcileOrphanedJobs`), a `LazyInitializationException` on retry (`findByIdWithPortfolioAndUser`), a 500 on `/spot-activity` from non-numeric deposit `externalId`s (`mapToTradeRow` now filters to BUY/SELL), and — most recently — a race between retrying an old `FAILED` job and triggering a fresh full sync of the same type, which collided at the `uk_sync_job_active` constraint. Fixed with a shared `ensureNoConflictingActiveJob` guard (used by both `createSyncJobs` and `retryJob`, plus a synchronous check in the retry endpoint for an immediate 409). 141 tests, same 11 pre-existing failures, 0 new. Also fixed the live-blocking `inventory_cost_usdt` column bug (migration 045) and an `exchange_name` backfill (046). Original PR #65 scope (resumable, chunked job tracking, `sync_job`/`sync_job_chunk`, spec in `docs/binance-sync-jobs.md`) opened 2026-09-15, still awaiting review — every fix above is documented incrementally in that same doc. [PR #63](https://github.com/fedefailla18/investracker/pull/63)/[#64](https://github.com/fedefailla18/investracker/pull/64) still open from before, awaiting review. |
