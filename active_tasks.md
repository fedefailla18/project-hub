...
- **`wealthtrack` [IN PROGRESS]**: **Bug — "Sync Data" doesn't refresh every screen.** `getSheetData()` (`lib/google-sheets.ts`) wraps all Sheets reads in `unstable_cache` with a 24h TTL, tag `"google-sheets"`. The "Sync Data" button (`app/components/AuthButton.tsx` → `usePortfolio().refresh()`) calls `revalidatePortfolioCache()` (`lib/actions.ts`), which only does `revalidateTag("google-sheets")` — no `revalidatePath()` for `/balance`, `/ledger`, `/accounts`, `/dashboard`, `/`, etc. Root cause and exact fix approach pending user's call on the freshness-vs-quota tradeoff (see question asked in-session 2026-09-06). Plan: add `revalidatePath()` for every route that reads sheet data, and confirm/adjust the 24h passive TTL.
- **`wealthtrack` [IN PROGRESS]**: **Bug — dates from the Apps Script snapshot automation render as raw serial numbers.** `getSheetData` fetches with `valueRenderOption: "UNFORMATTED_VALUE"`, so a genuine `Date`-typed cell (as written by `apps-script/wealthtrack-core.gs.js`'s `actualizarBalance()`/`generateNewSnapshot()`) arrives as a raw number (e.g. `46258`), not text. `lib/utils/date-utils.ts`'s `parseDate()` already converts that correctly to ISO — but `parseBalanceData()` in `lib/balance-transformers.ts` builds the *displayed* `date` field via `date.toString().trim()` instead of `parseDate(date)`, so older text-formatted rows ("22-julio-2026") display fine while new automation-generated rows show as "46258"/"46244". Same raw-value pattern found in `lib/registro-saldos-transformers.ts` (`parseAccountRaw`'s `date: date || ""`, and `getSnapshotRowsByDate`'s `dateDisplay` fallback) — likely affects Snapshot/Ledger pages too, not just `/balance`. Fix: route all display-path dates through `parseDate()` (converted to the sheet's existing `dd-mmmm-yyyy` Spanish display style, to avoid changing how older rows already look).
