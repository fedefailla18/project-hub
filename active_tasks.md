...
- **`importer-porfolio` [NEW]**: `./wp run docker`'s `crypto-ui` service (and the "Docker Nginx runner" migration claimed in the `✅ Completed` entry below) can't actually build — **`importer-porfolio` has no `Dockerfile`** (confirmed absent, `docker compose build crypto-ui` fails with "failed to read dockerfile"). Found while verifying the investracker/wp fix below; the backend+DB+Redis portion of `./wp run docker` now works end-to-end, but the frontend container image doesn't exist yet. Needs a CRA build → nginx-serve Dockerfile (+ nginx config for SPA routing) before `./wp run docker` can bring up the FE too.
